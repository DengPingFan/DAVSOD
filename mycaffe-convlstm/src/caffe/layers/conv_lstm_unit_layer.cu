...
template <typename Dtype>
__global__ void ConvLSTMActsForward(const int nthreads, const int dim,
...
template <typename Dtype>
__global__ void ConvLSTMUnitForward(const int nthreads, const int dim,
...
template <typename Dtype>
void ConvLSTMUnitLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  ConvLSTMActsForward<Dtype><<<CAFFE_GET_BLOCKS(X_count), CAFFE_CUDA_NUM_THREADS>>>(
      X_count, hidden_dim_, X, X_acts);
  CUDA_POST_KERNEL_CHECK;
  // NOLINT_NEXT_LINE(whitespace/operators)
  ConvLSTMUnitForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
      count, hidden_dim_, C_prev, X_acts, cont, C, H);
  CUDA_POST_KERNEL_CHECK;
}

template <typename Dtype>
__global__ void ConvLSTMUnitBackward(const int nthreads, const int dim,
...
template <typename Dtype>
__global__ void ConvLSTMActsBackward(const int nthreads, const int dim,
...
template <typename Dtype>
void ConvLSTMUnitLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  ConvLSTMUnitBackward<Dtype>  // NOLINT_NEXT_LINE(whitespace/operators)
  ...
  ConvLSTMActsBackward<Dtype>  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<CAFFE_GET_BLOCKS(X_count), CAFFE_CUDA_NUM_THREADS>>>(
      X_count, hidden_dim_, X_acts, X_acts_diff, X_diff);
  CUDA_POST_KERNEL_CHECK;
}

INSTANTIATE_LAYER_GPU_FUNCS(ConvLSTMUnitLayer);
